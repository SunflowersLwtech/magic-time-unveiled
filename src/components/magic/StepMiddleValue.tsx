...
const StepMiddleValue = () => {
  const { mode, getM, getT, getX, nextStep, prevStep, setShowResult } = useMagic();
    ...
    >
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          {mode === "demo" ? "见证奇迹的时刻..." : "神奇数字 M 的真实计算逻辑"}
        </p>
      </div>
      ...
      {!revealed ? (
        ...
        <AnimatePresence>
          ...
          >
            ...
            >
              <p className="text-sm text-muted-foreground">神奇数字 M</p>
            ...
            {mode === "decrypt" && (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground border-t border-border pt-3">
                  💡 神奇数字 M = T - X。无论你输入什么随机数，M 都会自动调整，使得 M + X 恒等于目标时间 T！
                </p>
              </motion.div>
            )}
